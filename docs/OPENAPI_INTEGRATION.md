...
## Version Compatibility (v1.6+)

Sannr 1.6+ supports both **Microsoft.OpenApi v1.x** (older Swashbuckle) and **v2.x** (Swashbuckle 10+). To ensure the source generator emits the correct schema mapping code, you should specify your version in your `.csproj`:

```xml
<PropertyGroup>
  <!-- Options: v2 (default for v2.x refs) or v3 (for v1.x models refs) -->
  <SannrOpenApiVersion>v2</SannrOpenApiVersion>
</PropertyGroup>
```

### SANN005: Version Mismatch Diagnostic
The Sannr Source Generator now includes an analyzer (SANN005) that proactively checks your project references. If it detects that your `SannrOpenApiVersion` configuration doesn't match the actual OpenAPI library referenced by your project, it will emit a build warning:

> **Warning SANN005**: Project is using a Microsoft.OpenApi version that likely does not match configured SannrOpenApiVersion 'v2'. Detected OpenAPI v1.x style references.

To resolve this, update your `<SannrOpenApiVersion>` to match your referenced `Swashbuckle` or `Microsoft.OpenApi` version.
...
- Ensure `options.SchemaFilter<Sannr.OpenApi.SannrGeneratedSchemaFilter>()` is called in SwaggerGen options
...
builder.Services.AddSwaggerGen(options =>
{
    options.SchemaFilter<Sannr.OpenApi.SannrGeneratedSchemaFilter>(); // Single line!
});
