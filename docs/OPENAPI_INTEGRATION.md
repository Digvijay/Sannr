...
Sannr allows customizing the OpenAPI generation via MSBuild properties in your project file (`.csproj`).

### 1. Enable Schema Generation (Opt-in)

By default, the source generator only emits schema filters if `builder.Services.AddSannr()` is detected or if explicitly enabled:
...
Swashbuckle.AspNetCore v10 introduced breaking changes by moving to the `Microsoft.OpenApi` v2.0 library. Sannr targets **OpenAPI v2 (Swagger 2.0)** by default to ensure maximum compatibility with the latest tools.

To switch back to OpenAPI v3 (e.g., for older Swashbuckle versions):

```xml
<PropertyGroup>
  <SannrOpenApiVersion>v3</SannrOpenApiVersion>
</PropertyGroup>
```

| Property | Default | Description |
| :--- | :--- | :--- |
| `EnableSannrSchemaGen` | `false` | Enables compile-time generation of OpenAPI filters. |
| `SannrOpenApiVersion` | `v2` | Target OpenAPI specification version (`v2` or `v3`). |
...
- Verify `<EnableSannrSchemaGen>true</EnableSannrSchemaGen>` is set in your `.csproj` OR `builder.Services.AddSannr()` is called
