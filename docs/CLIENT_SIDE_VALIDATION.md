...
After compilation, Sannr generates a static class with your validation rules as JSON:
...
```javascript
// In your JavaScript/TypeScript application
import { UserRegistrationFormValidators } from './generated-validators.js';

// Parse the JSON rules
const validationRules = JSON.parse(UserRegistrationFormValidators.ValidationRulesJson);

// Use with your favorite validation library
...
| `[RegularExpression(pattern)]` | `"pattern": "pattern"` | `{ "pattern": "^[A-Z]{2}\\d{8}$" }` |
